...
Recurring monthly invoice drafts, reconciliation and period locking are not implemented. Approval thresholds, second-person authorization for payouts, and the split between a finance operator and an authorized payer remain proposals. `invoice:approve` still covers issuing, voiding and writing off as one capability.

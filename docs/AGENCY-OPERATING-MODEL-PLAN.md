...
Phase 5 second slice lets the client record their own approval, instead of a staff member writing down what the client said. Migration `20260919000000` is applied.

The design decision that shaped everything else: a client gets a link, not an account. `app_is_internal()` is defined as "signed in, no partnerId, has a scope", so a client user created without a partnerId would read as internal staff and see the entire system. Rather than thread a second external identity type through every policy that already exists, a review link carries no identity at all - it is an unguessable token granting one content version for a limited time. The blast radius of a leaked link is one deliverable, not an account, and it expires on its own.

Only a SHA-256 of the token is stored. The token is generated in the application, shown to the sending staff member exactly once, and never written to the database; lookup is by hash, so a database copy yields no working links. If a token is lost, the link is withdrawn and another issued.

A link can only be sent for the current version of the current plan, on a plan that requires client approval, and only after internal approval - sending work the team has not signed off is how a half-finished draft reaches a customer. One live link per version. Staff can withdraw an unused link; a link that has already been used cannot be withdrawn, because that would hide a decision genuinely made. The client's answer is written into the existing `content_review` table with `stage='CLIENT'`, so every existing publication gate keeps working unchanged, and stamped with `viaLinkId` so an approval the client made themselves stays distinguishable from one a staff member transcribed. Both forms are labelled in the interface.

Validated by 29 unit tests, a rollback-transaction database suite, and 15 browser checks including a client browser with no cookies and no CRM account. The browser pilot confirmed that internal review notes, staff names, internal identifiers and the CRM navigation do not reach the client page, that the raw token appears nowhere in the database, that an invented link reveals nothing, that reopening a used link shows the decision rather than inviting a second answer, and that the client's own approval satisfies the existing publication gate. Postflight confirmed `anon` holds execute on exactly the two functions a client needs, none on issuing or revoking, and no table privilege on the link table.

Nothing is emailed: the staff member copies the link and sends it themselves, which is why the recipient's email is recorded for the audit trail rather than used for delivery. Anyone holding the link can use it, so the link is the credential and its expiry is the control; there is no per-client identity, no client-side history across deliverables, and no client view of anything other than the single version they were sent. A client portal with accounts, should the agency want one, would build on this rather than replace it.

Health reflects what has been recorded in the CRM, so an account nobody logs work against looks quiet rather than well. This is a review tool, not a churn prediction, and the weights are a starting point that should be revised once the team has used it. Named-manager assignment still uses the existing `ownerUserId`; a separate customer-success owner distinct from the sales owner, and a renewal ownership workflow, remain future work. Customer subscriptions were added to the renewal queue and to health in `20260919000002`; the sentence that previously said only contracts produce renewal dates no longer holds.

Phase 4 third slice splits `invoice:approve`, applied as migration `20260919000001`. One permission had quietly accumulated five powers: issuing an invoice, writing one off, closing an accounting month, approving a vendor bill, and receiving expense-approval notifications. Granting someone the first handed them all five, and asking a customer for money is close to the opposite of forgiving what they owe.

`invoice:issue`, `invoice:void`, `period:close` and `payable:approve` are now separately grantable. Nothing was taken away: `invoice:approve` remains the coarse grant and still satisfies each of them, through `app_can_issue_invoice()`, `app_can_void_invoice()` and `app_can_close_period()`, so no existing role lost an authority. Roles that held the coarse grant were additionally given the explicit set, so their permissions now say what they can do rather than implying it. No user's role assignment was changed; whether to move anyone onto a narrow grant is a business decision.

The work also found an existing defect. Expense approval moved onto `expense:approve` in `20260830000000`, but `notifyExpenseSubmitted` still selected recipients by `invoice:approve`, so submission emails went to whoever ran the receivables ledger rather than to whoever could actually decide the claim. Fixed here.

Verified in a rollback transaction, then with fourteen browser checks using each synthetic identity's own token: a drafter with only `invoice:write` still prepares invoices; `invoice:issue` alone issues but cannot write off or close a period; `invoice:void` alone cancels but cannot issue; `invoice:approve` still does both exactly as before; `period:close` alone opens the period screen while a drafter is told plainly it needs period-closing authority. Postflight confirmed three new functions, both split policies, the old combined policy gone, every coarse-grant role holding the explicit set, and no role having lost `invoice:approve`.

Phase 5 third slice adds the customer subscription layer from section E, applied as migration `20260919000002`. Product plans priced the catalogue and contracts drove recurring billing; what was missing was the layer between them - a customer's own agreement to a plan, at a negotiated price, for a quantity, with its own dates and renewal terms. Without it "BabulPOS, per user, per month" lived only on a quotation, and a customer going from ten users to fifteen was recorded nowhere.

`customer_subscription` holds the account, product, a snapshot of the agreed plan, quantity, unit price, currency, frequency, dates, auto-renewal, status and the last period billed. The plan is copied rather than referenced: a catalogue price change must never rewrite what an existing customer is paying, which the browser pilot verifies directly.

Two decisions were taken deliberately and are recorded here because they are commercial, not technical. **Billing is whole-period, not prorated:** a period bills the quantity in force on the day it started, so adding users mid-month applies from the next period. Changes are stored with their effective date so the history supports proration later, but nothing charges a part period - a wrong proration asks the customer for the wrong money. **Billing produces drafts:** the run raises `DRAFT` invoices that a person reviews and a second person issues, following the existing separation of duties rather than sending anything automatically.

A subscription is created as a draft and only bills once activated. Status moves DRAFT→ACTIVE→PAUSED/CANCELLED/ENDED with a recorded reason; cancelled and ended are terminal, so reinstating means a new agreement and the old history stays intact. Commercial terms can only be edited while a draft; after that they change through a recorded quantity change. A period already invoiced cannot have its quantity rewritten. A partial unique index on `(subscriptionId, periodStart)` refuses a second live invoice for the same period, and `mark_subscription_billed` only advances the marker once an invoice exists and never rewinds it. Managing agreements uses `opportunity:write`; billing them uses `invoice:write`.
...
Two deliberate separation rules need a second person: content review excludes the author, and invoice issuing excludes the preparer. Both are satisfied today only because the administrator counts as that second person - the project manager and the administrator hold `project:manage`, Finance and the administrator hold `invoice:approve`. The sales handoff is the one genuinely blocked by this, since only the administrator qualifies at both ends. All three behave as designed. Whether to widen those grants, or to have the administrator take part in the pilot, is a business decision and no grants were changed.

`npm run pilot:readiness` now derives this from the live grants and record counts rather than restating what was true when the runbook was written. It reads grants and counts only - no record content, emails or credentials - and reports each workflow as runnable or names what is missing. Run against the live database on 19 September it reported 2 of 10 workflows runnable: content planning and review between the project manager and the administrator, and the delivery handoff between them. That corrected the runbook's first draft, which had understated what was available by assuming a single project manager left content review with nobody to approve it.

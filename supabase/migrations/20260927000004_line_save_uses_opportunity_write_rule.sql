-- Saving a deal's lines uses the deal's own write rule.
--
-- save_opportunity_lines checked scope with app_visible_owner_ids(), written to
-- look like the opportunity's policy. It was not the same rule: the policy is
-- app_can_write_owned("ownerUserId"), which also lets a partner manager write a
-- deal their partner raised. So somebody who could close a deal as won could
-- not add the product it was won on - found by the end-to-end walkthrough.
--
-- Security definer functions bypass row-level security, so the check inside
-- has to BE the policy, not resemble it. It now calls the same function.
...
begin
  ...
  -- EXACTLY the rule the opportunity's own write policy applies, rather than
  -- a hand-written approximation of it. The approximation was stricter: it
  -- refused a partner manager who could update the deal directly, so the one
  -- screen meant to price a deal was the one screen that could not.

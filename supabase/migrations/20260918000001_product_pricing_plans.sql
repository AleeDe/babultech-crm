-- Plans belong to the same product, so projects and sales keep their product link.
ALTER TABLE public.product ADD COLUMN "pricingPlans" jsonb NOT NULL DEFAULT '[]'::jsonb
  CHECK (jsonb_typeof("pricingPlans") = 'array');
UPDATE public.product SET "pricingPlans" = jsonb_build_array(jsonb_build_object(
  'id', gen_random_uuid(), 'name', 'Standard', 'billingType', "billingType",
  'unitOfMeasure', "unitOfMeasure", 'standardPrice', "standardPrice", 'standardCost', "standardCost"
));
-- Keep the selected plan snapshot even if catalogue pricing changes later.
ALTER TABLE public.quote_line ADD COLUMN "productPlan" jsonb;
ALTER TABLE public.invoice_line ADD COLUMN "productPlan" jsonb;
...
begin
  ...
  insert into quote_line (
    id, "quotationId", "productId", "productPlan", description, quantity, "unitPrice",
    "discountPercent", "taxRateId", "lineTotal", "sortOrder",
    "createdAt", "updatedAt"
  )
  select gen_random_uuid(), v_new_id, l."productId", l."productPlan", l.description, l.quantity,

...
export async function getContentVersions(projectId: string, taskId: string, page = 1) {
 ...
 const [plan, versions, latest, manage] = await Promise.all([
 ...
 return { task, userId: user.id, plan: plan.data, canManage: manage.data === true, canWrite: manage.data === true || task.assignedUserId === user.id, latest: latest.data?.versionNumber ?? 0, page: safePage, count: versions.count ?? 0, versions: rows as unknown as ContentVersionRow[] };
}
